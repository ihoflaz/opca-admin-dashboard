...
// ** Third Party Imports - axios OpCa için gerekli değil
// import axios from 'axios'

// ** Types Imports
// Fake-db types removed - using local types instead
interface AppBarSearchType {
  id: number
  url: string
  icon: string
  title: string
    ...
    suggestions: [
      {
        ...
        suggestion: 'OpCa Dashboard',
        link: '/opca/dashboard'
      },
      {
        icon: 'tabler:microscope',
        suggestion: 'Analiz Sonuçları',
        link: '/opca/analysis/list'
      },
      {
        icon: 'tabler:virus',
        suggestion: 'Parazit Yönetimi',
        link: '/opca/parasites'
      },
      {
        icon: 'tabler:users',
        suggestion: 'Kullanıcı Yönetimi',
        link: '/opca/users'
      }
    ]
  },
      ...
      <List sx={{ py: 0 }}>
        <ListItem sx={{ py: 2 }} disablePadding onClick={() => setOpenDialog(false)}>
          ...
          >
            ...
            <Typography>OpCa Dashboard</Typography>
          </Box>
        </ListItem>
        <ListItem sx={{ py: 2 }} disablePadding onClick={() => setOpenDialog(false)}>
          <Box
...
const AutocompleteComponent = ({ hidden, settings }: Props) => {
  ...
  // Get all data using API - OpCa için devre dışı bırakıldı
  useEffect(() => {
    // OpCa projesi için arama API'si olmadığı için boş array set ediyoruz
    setOptions([])
  }, [searchValue])
